...
from prompts.prompts import prompt_extraction_current, prompt_extraction_qa, prompt_refining_items
...
class ContrieverGraph(TripletGraph):
    def __init__(self, model, system_prompt, api_key, device = "cpu", base_url=None, price_per_1m=None):
        super().__init__(model, system_prompt, api_key, base_url=base_url, price_per_1m=price_per_1m)
    ...
    def update(self, observation, observations, plan, prev_subgraph, locations, curr_location, previous_location, action, items1, log, topk_episodic):        
        ...
        prompt = prompt_extraction_qa.format(observation = observation, example = example)
