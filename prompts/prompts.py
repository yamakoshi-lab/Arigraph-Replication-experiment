...
prompt_extraction_qa = '''Objective: The main goal is to meticulously gather factual information from the given text paragraph and organize this data into a clear, structured knowledge graph.

Guidelines for Building the Knowledge Graph:

Creating Nodes and Triplets: Nodes should depict entities or concepts, similar to Wikipedia nodes. Use a structured triplet format to capture data, as follows: "subject, relation, object".
...
Remember that you should break complex triplets like "John, position, engineer in Google" into simple triplets like "John, position, engineer", "John, work at, Google".
However, do not drop important descriptive words or adjectives (e.g. keep "NBA scoring title" instead of shortening to "title"). You should extract concrete factual knowledge.

Do not miss important factual information.
Several triplets can be extracted, that contain information about the same node. For example 'apple, is on, table', 'apple, color, red'.
Do not use 'none' as one of the entities.
...
Remember that triplets must be extracted strictly in format: "subject_1, relation_1, object_1; subject_2, relation_2, object_2; ..."
